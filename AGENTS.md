...
- Write repository content technically. Commit messages, READMEs, code comments and docs state what the code does and its technical reasons in the present tense. Do not record history such as earlier implementations, past incidents, former owners, or the PRs and issues that led to the current state; Git history and issues hold that record.
...
- All nine established integrations participate in native Changesets versioning and publication. Keep established packages out of `ignore`; verify any future package's first publication and trusted publisher before enabling its automated publication. Do not introduce a custom publication wrapper.
- A new package that does not exist on npm stays in Changesets `ignore` until its owner-authenticated first publication and trusted publisher are verified; `docs/releases.md` lists held packages (currently `nestjs-slightly-better-auth`, #534) and the procedure. Write Changesets for held packages in files naming only held packages: a mixed file fails `changeset version` and blocks every release.
- `nestjs-slightly-better-auth` releases `1.0.0`, which covers the HTTP scope of its reviewed design; GraphQL, WebSocket and RPC transports and its conformance kit follow as minor versions. It is held like any other unpublished package until its owner-authenticated first publication and trusted publisher are verified. Follow its package instructions and preserve its MIT licenses and historical design material.
