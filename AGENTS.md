...
- Every package's npm trusted publisher is registered with the workflow filename `release.yml`. Never rename or move `.github/workflows/release.yml`, and never move publication into another workflow: npm accepts OIDC publication only from the registered workflow file, so any other file fails publication for every package.
