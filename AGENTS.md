...
- All twelve established integrations participate in native Changesets versioning and publication. Keep established packages out of `ignore`; verify any future package's first publication and trusted publisher before enabling its automated publication. Do not introduce a custom publication wrapper.
- A new package that does not exist on npm stays in Changesets `ignore` until its owner-authenticated first publication and trusted publisher are verified; `docs/releases.md` lists held packages (none currently) and the procedure. Write Changesets for held packages in files naming only held packages: a mixed file fails `changeset version` and blocks every release.
