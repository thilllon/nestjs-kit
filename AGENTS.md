...
- A new package that does not exist on npm stays in Changesets `ignore` until its owner-authenticated first publication and trusted publisher are verified; `docs/releases.md` lists held packages (currently `nestjs-strategy`, #572) and the procedure. Write Changesets for held packages in files naming only held packages: a mixed file fails `changeset version` and blocks every release.
- `nestjs-slightly-better-auth` published `1.0.0`, which covers the HTTP scope of its reviewed design. Its `./websockets` and `./microservices` entries add WebSocket and RPC transports, and `./testing` and `./testing/conformance` provide the consumer testing helpers and conformance kits; the GraphQL transport follows as a minor version. It releases like every other package. Follow its package instructions and preserve its MIT licenses and historical design material.
