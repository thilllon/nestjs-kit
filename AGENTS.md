...
- `nestjs-slightly-better-auth` published `1.0.0`, which covers the HTTP scope of its reviewed design. The package also provides the consumer testing helpers (`./testing`) and conformance kits (`./testing/conformance`); GraphQL, WebSocket and RPC transports follow as minor versions. It releases like every other package. Follow its package instructions and preserve its MIT licenses and historical design material.
