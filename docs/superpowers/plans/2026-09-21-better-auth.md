...
- [x] Run `mise exec -- pnpm exec vitest run --config vitest.e2e.config.mts packages/nestjs-slightly-better-auth/src/graphql-http.e2e.test.ts`; verify failure. Implement driver boot validation, positive live-platform HTTP recognition, cookie sink mapping, shared HTTP principal key and distinct field invocation identities.
