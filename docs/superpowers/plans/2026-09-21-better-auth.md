...
Nest GraphQL 14.0.1's code-first federation factory is incompatible with Apollo subgraph 2.15.0 and 2.15.1. Independent native probes verified 2.14.4 as the latest compatible stable version for both federation modes on Nest 12.0.3; development fixtures pin it exactly. [#552](https://github.com/thilllon/nestjs-kit/issues/552) records the version boundary and an owner-reviewable upstream draft. Exercise authenticated schema-first and code-first Apollo/Mercurius federation against this pair. Do not patch vendor internals or substitute a mock executor; broader versions still require the Task 12 matrix.
