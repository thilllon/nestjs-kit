...
- Every implementation PR links its checklist issue and includes an explicit Changeset for publishable package changes. Do not append `Co-Authored-By` trailers. Merge only protected, verified revisions; use `gh pr merge --body ''`.
...
- [ ] Freeze the reviewed v7 commit and the dependency-source evidence; root records SDK 1.7.5 source commit `5468e6bfcdff799848537cf5ad06ebab15aad9dd` and exact installed package versions.
...
- Add the task's explicit Changeset under `.changeset/`; retain package privacy.
...
- [ ] Commit the scoped changes and Changeset with `feat(auth): add typed contracts safe errors and isolated tokens`; keep this task open until typecheck and contract review pass.
...
- [ ] Add the binding regression below. Construct a complete test-local `BridgeBinding` with all 12 database targets (`user|session|account|verification` × `create|update|delete`) mapped to empty before/after arrays, empty endpoint arrays, no credentials, `current: () => undefined`, and `onDropped: () => undefined`.
...
- [ ] Run `mise exec -- pnpm exec vitest run packages/nestjs-slightly-better-auth/src/plugin.test.ts packages/nestjs-slightly-better-auth/src/hook-dispatcher.test.ts packages/nestjs-slightly-better-auth/src/database-hook-dispatcher.test.ts` and package typecheck. Commit with the task Changeset as `feat(auth): add construction-time hook and cookie bridge`.
...
- [ ] Write this lazy-capability regression in `request-scope.test.ts`, importing `RequestScope`, `ScopeView`, and Task 1's `BetterAuthConfigurationError`:
...
- [ ] Differentially compare cookie/form verdicts with real SDK routes, including cross-site no-cookie GET form forwarding (denied before endpoint effects), same-origin success, no-cookie form navigation, null/missing origins, trustedOrigins function/plugin contribution, path-array skips and boolean skip combinations. Set `advanced.disableOriginCheck: false` explicitly in security fixtures; Vitest's `TEST=true` must not invalidate the test.
- [ ] Implement `platform.ts` helpers from v7 §2.2.10. Add `platform.test.ts` covering bounded Node/Web streams, declared/chunked overflow, aborts, pseudo-header filtering, individual cookies, HEAD/backpressure and TLS-plus-Host upgrade URLs. These are stream/object tests without a listening server.
- [ ] Run `mise exec -- pnpm exec vitest run packages/nestjs-slightly-better-auth/src/request-scope.test.ts packages/nestjs-slightly-better-auth/src/principal-readings.test.ts packages/nestjs-slightly-better-auth/src/origin-check.test.ts packages/nestjs-slightly-better-auth/src/trusted-origins.test.ts packages/nestjs-slightly-better-auth/src/platform.test.ts` and package typecheck, then commit with Changeset as `feat(auth): isolate request state and enforce origin-safe scopes`.
...
- [ ] Start with a real Nest injection/isolation failure in `auth-module.test.ts`:
...
- [ ] Start the red regression in `auth-readers.test.ts` using a real module application context from Task 4a. Outside a handler scope, obtain its actual `BetterAuthService` and assert:
...
- [ ] Run targeted unit/E2E tests, package typecheck/build, and Tasks 4a–4b regression tests after integration. Commit with Changeset as `feat(auth): preserve Express auth body and cookie semantics`.
...
- [ ] Run both Fastify E2E files plus shared helper/exchange tests and package typecheck. Compare Express/Fastify outputs for raw bytes, limits, cookies, CORS, method routing and filter behavior. Commit with Changeset as `feat(auth): add isolated Fastify and HTTP2 support`.
...
- [ ] In `api-key.test.ts`, build a real memory-backed SDK auth with `apiKey()` and the library plugin, create a user and a key granting only `{ project: ['read'] }`, and resolve it through the real source/handle fixture. Assert this boundary before implementing it:
...
- [ ] Run the three GraphQL E2E files, lineage unit tests and package typecheck; commit with Changeset as `feat(auth): support GraphQL operations subscriptions and federation`.
...
- [ ] Run both socket E2E files, unit tests and typecheck; commit with Changeset as `feat(auth): secure socket messages and connection authentication`.
...
- [ ] Run carrier unit tests, `mise exec -- pnpm docker:up`, the three RPC E2E files, `mise exec -- pnpm docker:down` in cleanup and package typecheck. Commit with Changeset as `feat(auth): authenticate standalone and hybrid RPC transports`.
