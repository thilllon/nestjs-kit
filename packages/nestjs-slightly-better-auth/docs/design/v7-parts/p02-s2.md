...
**Flat active source, logical boundaries** `[MONOREPO]`. Implementation and colocated `*.test.ts` files live directly in package `src/`; no `src/lib/`, `src/core/` or nested extension tree is introduced. Filename groups (`kernel-*.ts`, `unit-*.ts`, `contracts*.ts`, `shared-*.ts`) preserve §3’s dependency direction. Subpath entry files such as `express.ts`, `graphql.ts`, `plugin.ts` and `testing-conformance.ts` export their own groups; architecture tests inspect these import graphs. Directory diagrams below represent logical units, not permission to recreate the historical directory layout.
...
export declare class BetterAuthService<A extends AuthLike = RegisteredAuth> {
   ...
   * whose origin has no passing verdict (including an advisory safe-leg verdict), as in a @Public() handler, throws
  ...
  /** Session projection: session kind → session; no-identity/absent → null; authenticated other kind → SESSION_REQUIRED. Same scope rules as getPrincipal(); reader errors deduplicate per request (§7.8). */
  ...
  /**
   * Web Headers for a direct auth.api call from a platform request: pseudo-headers dropped, client-IP header set. A state-changing proxy uses POST with @OptionalAuth() and a trusted Origin; forwarding auth cookies also requires
   * @ForwardAuthCookies(). A guard run on a safe method is insufficient without a passing advisory origin verdict
 ...
 * Because such a handler can sign a browser in, EVERY method and operation gets the enforcing form-CSRF rule (origin check mode 'form',
 * §7.10), also on @Public() routes and without a cookie, as better-auth's own sign-in routes do. That check runs in the guard, so
 * B16 counts forwarding as declaring something and does not treat public or inherited form access as coverage: under globalGuard: false the
...
// Injection uses Nest's @Inject() directly with the package-specific helpers in §2.2.8.
// No injection-decorator wrappers are exported. [MONOREPO]
```

#### 2.2.4 Built-in extension units exported from the root

These implement public contracts. Only the composition root (`src/better-auth.module.ts`, `src/index.ts`) references them; core never does (§3.2).
...
```ts
export declare function getBetterAuthInstanceToken(alias?: string): string;
export declare function getBetterAuthOptionsToken(alias?: string): string;
export declare function getBetterAuthServiceToken(alias?: string): string;
export declare function getBetterAuthHandleToken(alias?: string): string;
export declare const BETTER_AUTH_INSTANCE: "NESTJS_SLIGHTLY_BETTER_AUTH_INSTANCE";
export declare const BETTER_AUTH_OPTIONS: "NESTJS_SLIGHTLY_BETTER_AUTH_OPTIONS";
export declare const BETTER_AUTH_SERVICE: "NESTJS_SLIGHTLY_BETTER_AUTH_SERVICE"; // useExisting BetterAuthService
...
| Runtime brands and slots     | `bridge` (protocol object on the plugin), `extension` (ExtensionDefinition brand), `failure` (AuthFailure brand), `error` (brand on `BetterAuthInfrastructureError` and `BetterAuthConfigurationError`), `enhancer` (brand on `BetterAuthGuard` and `BetterAuthScopeInterceptor` instances; B16 recognizes the app's global enhancers by identity with the instances the class tokens resolve to, overrides included, and uses the brand only for an enhancer registered from the other package copy, §5.4) `[R3:NEST-r3-02]` `[R4:NEST-r4-02]`, `resolution` (readings the guard records: the invocation's own result on the invocation object, and readings by lineage position on the operation carrier, §7.8) `[R3:NEST-r3-01]`, `invocation` (per-invocation values on an invocation object), `raw-body` (Express capture), `upgrade-request` (ws), `mounted` (per-server double-mount stamp), `raw-cause` (non-enumerable slot on infrastructure errors), `test-app` (slot on the plugin handle used by `initTestApp`), `surfaced` (shared operation/invocation record for once-per-request error delivery, §7.8), `test-stamp` (`stampPrincipal()` record on an args array and its elements, §7.8). Extension units own further keys under the same prefix, for example `org:active-organization-id` and `org:active-member-role` (`./organization`) |

**Public registration tokens** `[MONOREPO]`. Nest injection uses `@Inject(getBetterAuthInstanceToken(alias))`, `@Inject(getBetterAuthServiceToken(alias))`, and `@Inject(getBetterAuthOptionsToken(alias))` directly. Omitted alias (or the reserved internal name `default`) returns the unchanged base token; a named registration appends `_<alias>`, for example `NESTJS_SLIGHTLY_BETTER_AUTH_SERVICE_admin`. Reject an empty alias and duplicate named registrations at boot. Async aliases remain in module extras, outside `useFactory`; options, credentials, mounts, service instances and cleanup belong to that alias. `MODULE_OPTIONS_TOKEN` stays private and must not escape through barrel exports. Internal protocol/metadata `Symbol.for` identities remain intact; no custom injection decorator or legacy `getAuthToken(kind, name)` API is introduced. CJS/ESM consumer tests exercise default/named coexistence and shutdown.
