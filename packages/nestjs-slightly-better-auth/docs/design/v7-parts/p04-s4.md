...
export interface HttpRequestAccessor {
   ...
   * It is a security defense, not a convenience, and two lines on every platform. [R4:SEC-r4-03] [R5:SEC-r5-03]
   */
...
export function honoPlatform(
  ...
  const ctxOf = new WeakMap<HonoRequest, Context>(); // filled in phase 1; isLive is required (§4.1.1) [R5:SEC-r5-03]
  ...
  return defineHttpPlatform({
    ...
    prepare(ctx) {
      ctx.adapter
        ...
        .use("*", async (c: Context, next: () => Promise<void>) => {
          const raw = c.req.raw;
          ctxOf.set(c.req, c); // so requests.isLive() can read c.finalized
    ...
    requests: {
      ...
      isLive: (req) => ctxOf.get(req as HonoRequest)?.finalized === false, // required (§4.1.1): Hono's own "response sent" flag [R5:SEC-r5-03]
...
export interface AuthTransport {
   ...
   * getters (headers, browser, cookies, request, clientIp) throw the original configuration error only when consumed. [R6:SEC-r6-02]
   */
   ...
   * reference resolver) is never nested. The planner asks every registered transport; the first answer wins. [R2:NEST-r2-04]
   * [R3:NEST-r3-05] [R4:NEST-r4-01]
   */
   ...
   * nesting omit it. [R3:NEST-r3-01] [R4:NEST-r4-01]
   */
...
export interface TransportCall {
   ...
   * values. Never a connection. [R1:NEST-r1-01] [R4:NEST-r4-01] [R5:NEST observation]
   */
   ...
   * strips any inbound client-IP header and sets it from clientIp. Throws a request-time configuration error when extraction is unavailable; public scope creation never calls it. [R4:BA-r4-03] [R6:SEC-r6-02]
   */
  headers(): Headers;
...
export interface InvocationLineage {
   ...
   * the recorded carrier metadata, supplying their own args (never a closure over a sibling/finished request). [R6:SEC-r6-02]
   */
 ...
 * [R3:NEST-r3-02] [R4:NEST-r4-02] [R7:NEST-r7-01]
...
- the guard reads `toException`, `toInternalException`, `invocation` and `lineage` (readings), and `lineage()` on public and inherit plans;
...
| T5  | `headers()` never throws; missing credentials give empty `Headers`. Unless the transport declares `requires.hostlessCalls`, they carry the leg's `host` (and `x-forwarded-host`, `x-forwarded-proto` when present) for every credential mapping it offers, user overrides included `[R4:BA-r4-03]`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `T-no-credentials`, `T-dynamic-base-url`                                                                                                                                                                                                                                                                                                               |
| T6  | `lineage`, when present: `carrier` is one of `ctx.getArgs()` and belongs to one operation (never shared across concurrent requests); `position` is distinct for distinct invocations of the operation (aliases, list items, batched operations), except that invocations of one handler plan the operation reaches as parallel entry points may share one (in the GraphQL units, the federation representations of one entity type), and equal for every guard run of one invocation; `enclosing(args)` yields exactly the positions of the enclosing invocations, nearest first, and each of them completes its guard run before the nested invocation starts. A nested reader therefore sees the reading of its own enclosing invocation, whatever its siblings select and in whatever order their guards settle (§7.8) `[R2:NEST-r2-06]` `[R3:NEST-r3-01]` `[R4:NEST-r4-01]`. | `T-carrier`, `T-stamp-per-plan`, `T-reference-resolver`                                                                                                                                                                                                                                                                                                |
| T7  | `invocation` is distinct for every handler invocation (two aliased GraphQL root fields, two batched operations, two WS messages on one socket, two federation representations in one `_entities` call) and identical for every guard run of the same invocation `[R1:NEST-r1-01]` `[R4:NEST-r4-01]`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `T-invocation-decisions`, `T-reference-resolver`                                                                                                                                                                                                                                                                                                       |
| T8  | `browser` is present whenever a browser can reach the operation. `enforce` is the cookie-mode classification only (form mode always enforces), and is true for every operation a WebSocket carries, whatever its GraphQL operation kind. `browser.headers()` are the leg's own headers: credentials a transport takes from connectionParams, `handshake.auth` or payloads never appear there. `url` is absolute, built from the socket's TLS state and `Host` for WebSocket legs (never from forwarding headers), and `key` identifies the leg. A GraphQL operation is treated as carried over HTTP only when its request is positively the platform's (`isRequest`) and still unanswered (`isLive`, required since v6) `[R1:SEC-r1-01, BA-r1-03]` `[R2:SEC-r2-01, SEC-r2-03, SEC-r2-05]` `[R3:SEC-r3-02, SEC-r3-05]` `[R4:SEC-r4-03]`.                                          | `T-csrf-http-unsafe`, `T-csrf-http-cookie-plus-token`, `T-csrf-safe-methods`, `T-csrf-login-proxy`, `T-ws-origin-untrusted`, `T-ws-origin-junk-token`, `T-ws-origin-dynamic-baseurl`, `T-ws-origin-forwarded-host`, `T-ws-origin-function-trusted-origins`, `T-subscription-origin`, `T-subscription-origin-junk-connection-params`, `T-stale-context` |
...
| T10 | `validate` claims every handler the transport serves, with the reach that matches the running app and the inputs it knows (§5.4 B16), entry points such as federation reference resolvers included; `defaultAccessFor` answers only for handlers whose access an enclosing, already authorized operation decides; every Nest-owned metadata literal the unit reads is checked against the installed Nest by a canary before it is used (§12.4) `[R2:NEST-r2-02, NEST-r2-04]` `[R3:NEST-r3-08, NEST-r3-10]` `[R4:NEST-r4-01]`.                                                                                                                                                                                                                                                                                                                                                    | `T-coverage-claims`, `T-inherit-no-lookup`, `T-metadata-canary`, `T-reference-resolver`                                                                                                                                                                                                                                                                |

**T11 — public scope construction is structural** `[R6:SEC-r6-02]`. For a context accepted by `handles`, `describe` and structural lineage extraction must not require a recognized, live credential request. Defer those checks to request-dependent accessors; a successful guarded extraction yields the same keys as before. For an unclassifiable GraphQL envelope use its raw context as the temporary request key and its `info`/representation as invocation; for ws use the per-message args. These fallback keys carry no authenticated reading. An inherited authenticated reading requires the transport’s live-request validation before it is returned. A scope must preserve deferred errors, never silently become unscoped. Conformance: `T-public-unclaimed-context`, `T-stale-context`, `T-public-direct-invalid-context`.
...
- `select(ctx)` returns the first transport whose `handles(ctx)` is true, and throws `NO_TRANSPORT` when none does (below). `find(ctx)` is the same lookup without throwing: it returns `undefined` when no transport handles the context, for the `inherit` and `public` plans that need a transport only if one exists. To change HTTP error bodies, list a subclass of the HTTP transport; it is selected before the built-in one `[A]`.
- The guard needs a transport only for plans that resolve or check an origin. An `inherit` plan without form forwarding asks the first transport that handles the context for its `lineage()` only, and fails with `NO_ENCLOSING_DECISION` when a lineage exists but no enclosing position holds a reading, which is a nested handler reached through an entry point no guard ran for (a federation entity without a guarded reference resolver, §9.2) `[R4:NEST-r4-01]`. A `public` plan without the form origin check asks the same for its `lineage()` only, to record "no identity" for the invocations nested in it unless the plan is itself nested (§7.8) `[R4:NEST-r4-03]`, and returns true whether a transport handles the context or not. A public handler in a context no transport serves (a third-party context type such as `@golevelup/nestjs-rabbitmq`'s `'rmq'`, which global guards reach, REV2-NEST:`exp3-external-context-global-guard.mjs`) therefore still does no auth I/O and gets no error `[R2:NEST-r2-03]` `[R3:NEST-r3-01]`.
  ...
  defineTransport({
    ...
    describe: (ctx, kit) => {
      ...
      return {
        ...
        browser: {
          enforce: !["GET", "HEAD", "OPTIONS"].includes(request.method), // §4.2.1: safe methods are not enforced (T8)
          headers: () => http.headers(req),
          url: request.url,
          key: http.key(req),
        },
...
export interface PrincipalSource<P extends AuthPrincipalBase = AuthPrincipal> {
   ...
   * S-delegation checks the declaration against the principals the source produces. Default false. [R4:NEST-r4-04]
   */
...
export interface AuthHandle<A extends AuthLike = AuthLike> {
  ...
  /** Origin/form rule for a browser leg (§7.10): null on pass/nonapplication, else denial; memoized per leg. browser.enforce gates cookie mode only; false never bypasses form mode. [R2:SEC-r2-01, SEC-r2-08] [R7:SEC-r7-01] */
  checkOrigin(
...
export interface AuthContextView {
  ...
  /** better-auth's resolved rate limiter; B30 reports it off in production (LEAD-V52). [R4:BA-r4-02] */
  ...
  readonly options: {
    ...
    /** `enabled` is undefined unless the user set it (B30 tells an explicit false from better-auth's NODE_ENV default). [R4:BA-r4-02] */
    readonly rateLimit?: { readonly enabled?: boolean };
  ...
  /** The admin policy's owner-row read for delegated principals (the ban rule), the call userHasPermission itself makes (LEAD-V45). [R3:BA-r3-02, SEC-r3-06] [R4:BA-r4-01] */
...
| P5  | A principal whose rights are narrower than its owner's carries `delegation`, and `delegation.allows` never grants more than the credential's own grant `[R1:BA-r1-01]`. A source declares `delegates` exactly when its principals carry `delegation` `[R4:NEST-r4-04]`.                                                                                                                                                                                                                                                                                           | `S-delegation`                               |
...
export const oauthAccessTokenPrincipal = (
  ...
  definePrincipalSource<OAuthAccessTokenPrincipal>({
    ...
    delegates: true, // its principals carry delegation (B15) [R4:NEST-r4-04]
...
A JWT-plugin source follows the same error rule, but it must not use `auth.api.verifyJWT`: that endpoint catches every error, a failing `jwks` read included, and answers `payload: null` exactly as for a forged token (LEAD-V53), so a storage brownout would answer 401 `INVALID_JWT` to every valid token, and machine clients would discard their tokens while monitoring sees 4xx `[R4:BA-r4-04]`. It verifies the way better-auth's JWT documentation shows for resource servers, with jose (LEAD-EXP-12) `[B][C]`:

```ts
// a JWT-plugin principal source (sketch): keys from better-auth's /jwks endpoint, cached by the source instance
const REJECT = new Set(['ERR_JWT_EXPIRED', 'ERR_JWT_CLAIM_VALIDATION_FAILED', 'ERR_JWT_INVALID', 'ERR_JWS_INVALID',
  'ERR_JWS_SIGNATURE_VERIFICATION_FAILED', 'ERR_JOSE_ALG_NOT_ALLOWED', 'ERR_JWKS_NO_MATCHING_KEY', 'ERR_JWKS_MULTIPLE_MATCHING_KEYS']);
async resolve(r) {
  const token = /^Bearer\s+(\S+)$/i.exec(r.headers.get('authorization') ?? '')?.[1];
  if (!token) return absent();
  // iss/aud are the baseURL ORIGIN, not AuthContextView.baseURL (which carries the basePath). [R5:BA-r5-01]
  const { baseURL } = await r.auth.context(); // AuthHandle view of the shared boot context
const baseOrigin = baseURL ? new URL(baseURL).origin : undefined;
const issuer = this.options.issuer ?? baseOrigin;
const audience = this.options.audience ?? baseOrigin;
if (!issuer || !audience) {
  throw BetterAuthConfigurationError.atRequest('JWT_CLAIMS_NOT_CONFIGURED',
    'Set jwt.issuer and jwt.audience on better-auth and matching issuer/audience on this source when baseURL is unset or dynamic.');
}
this.issuer ??= issuer;
this.audience ??= audience;
  const jwks = await this.keys.get(() => r.auth.api.getJwks({ headers: r.headers }));          // a throw (storage down) propagates: 5xx
  try {
    const { payload } = await jwtVerify(token, createLocalJWKSet(jwks), { issuer: this.issuer, audience: this.audience });
    return authenticated({ kind: 'jwt', source: 'better-auth:jwt', userId: typeof payload.sub === 'string' ? payload.sub : null, claims: payload });
  } catch (e) {
    const code = (e as { code?: string }).code;
    if (code === 'ERR_JWKS_NO_MATCHING_KEY' && this.keys.stale()) return this.resolve(r);  // refetch once after key rotation
    if (code && REJECT.has(code)) return rejected(AuthFailures.rejected({ status: 401, reason: 'INVALID_JWT', challenge: 'Bearer error="invalid_token"' }));
    throw e;                                                              // anything else is infrastructure (S6)
  }
}
```

**The claim check must use the baseURL _origin_, not `AuthContextView.baseURL`** `[R5:BA-r5-01]`. better-auth's JWT plugin signs with `iss = options?.jwt?.issuer ?? baseURLOrigin` and `aud = options?.jwt?.audience ?? baseURLOrigin`, where `baseURLOrigin` is the dispatch’s `ctx.context.options.baseURL` (`BA/plugins/jwt/sign.ts:289-302`); for a static configuration the shared `$context.options.baseURL` is `new URL(baseURL).origin` — **without** the basePath (`BA/context/create-context.ts:190-196`). The only base URL this design names elsewhere is `$context.baseURL`, which §6.2 uses for the mount and §2.2 exposes as `AuthContextView.baseURL`, and that one _does_ carry the basePath (`http://localhost:3000/api/auth`). Verifying with it fails every well-formed, unexpired, correctly signed token with jose's `ERR_JWT_CLAIM_VALIDATION_FAILED`, which the `REJECT` set maps to `rejected(401 INVALID_JWT)`: the source would fail closed on every machine client, and the symptom would look like a signing problem (REV5-BA:`exp1-jwt-issuer.mjs`, identical on 1.7.0 and 1.7.4; LEAD-V57). Two consequences for a source author:

- derive the static base origin only when `$context.baseURL` is nonempty; derive issuer and audience independently, using the source’s `issuer` and `audience` overrides when present. Validate missing claims in the source’s boot validation with `JWT_CLAIMS_NOT_CONFIGURED`; the sketch retains the same request-time guard for consumers who use it without that validation. No `new URL('')` is evaluated. `[R6:BA-r6-03]`
- an empty **shared boot context** baseURL is not the signing context: a dynamic configuration resolves a per-call context from allowed host headers or its configured fallback, and JWT defaults read that clone’s `options.baseURL`. The recorded 1.7.5 probe minted dynamic-host defaults `https://tenant.example`, while an unset-baseURL direct call retained empty defaults; HTTP handler contexts can be resolved too. This source therefore **requires explicit stable issuer and audience whenever its shared baseURL is empty**, rather than guessing signing values from headers or token claims. Configure `jwt: { issuer, audience }` and the same source values; this is the source’s contract, not a claim that every empty shared context signs empty claims. `[R7:BA-r7-01]`
- key retrieval is `auth.api.getJwks({ headers: r.headers })`, so the wrapped endpoint can resolve a dynamic baseURL with no fallback. `PrincipalRequest.headers` already preserves host and platform-approved forwarded-host/protocol information (§4.3.1); pass that existing header object through the same internal scope without stripping credentials, rewriting origin/cookie headers, disabling hooks, or changing forwarding. The cache remains owned by the registered source instance, so credentials, keys and cleanup never cross aliases. A transport that cannot supply a resolution source needs a validated dynamic fallback under the existing hostless-call rule (B20); do not silently invent a host. JWKS storage/resolution errors remain infrastructure failures, while cryptographically invalid or wrong-claim tokens remain denials. Never derive expected issuer/audience from the token. `[R7:BA-r7-01]`

A service that is not the better-auth host uses `createRemoteJWKSet(new URL('/api/auth/jwks', <the same origin>))` instead — the origin again, with the basePath spelled out in the path — whose fetch and timeout errors carry no verification code and therefore throw. The source declares no `delegates` (a JWT the JWT plugin issued carries the session user's identity, not a narrower grant) unless its claims map to one. The upstream proposal (RK18) also asks `verifyJWT` to rethrow errors that are not verification failures.
...
> {
  ...
  readonly requires?: {
     ...
     * exactly the kinds of the instance's sources that do not declare `delegates` (B15). [R1:SEC-r1-02] [R4:NEST-r4-04]
     */
...
export class ProjectOwnerPolicy implements AuthorizationPolicy<{
  ...
  // and B15 fails boot for a route that admits only delegated kinds and uses this policy [R4:NEST-r4-04].
