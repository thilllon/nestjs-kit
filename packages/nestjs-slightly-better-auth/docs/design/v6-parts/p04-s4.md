...
export interface TransportCall {
   ...
   * strips any inbound client-IP header and sets it from clientIp. Never throws. [R4:BA-r4-03]
   */
  headers(): Headers;
 ...
 * whoever registered them (a forRoot with globalGuard: true, or the app's own APP_GUARD / APP_INTERCEPTOR) and whatever a test override
 * replaced them with (§5.4 B16). [R3:NEST-r3-02] [R4:NEST-r4-02]
...
async resolve(r) {
  ...
  this.issuer ??= this.options.issuer ?? new URL((await r.auth.$context).baseURL).origin;
  const jwks = await this.keys.get(() => r.auth.api.getJwks());          // a throw (storage down) propagates: 5xx
  try {
    const { payload } = await jwtVerify(token, createLocalJWKSet(jwks), { issuer: this.issuer, audience: this.issuer });
