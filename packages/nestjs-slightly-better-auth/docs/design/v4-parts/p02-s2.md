...
export interface RoutePlan {
   ...
   * guard returns true with no I/O, and readers take the nearest enclosing invocation's reading (§7.8). [R2:NEST-r2-04] [R3:NEST-r3-05]
   */
...
export declare class BetterAuthService<A extends AuthLike = RegisteredAuth> {
   ...
   *   inherit plans → the reading of the nearest enclosing invocation (lineage), else a test stamp, else NO_AUTH_RESULT;
   ...
   * The reading belongs to the plan's instance, which `instance` names, whichever instance this service serves.
   */
...
/** What principalFor(ctx) reads: the guard's result, or 'no-identity' where the plan resolves none (public plans, and inherit plans under a public operation). [R3:NEST-r3-02] */
...
export interface ResolutionRequest {
   ...
   * principal (its id), which must declare sessionBacked, reading and writing no memo entry. [R2:BA-r2-03] [R3:BA-r3-01]
   */
...
export interface GraphqlTransportOptions {
  ...
  /** Full override of credential extraction for operations over a WebSocket. [B][C] */
 ...
 * own framework and fails with GRAPHQL_DRIVER_MISMATCH otherwise (§9.2). [R3:NEST-r3-09]
 */
 ...
 * cookie cache. It then reads the stored user row (the findUserById call userHasPermission itself makes) and passes that row's role:
 * better-auth's adminMiddleware evaluates the stored role, never a customSession shape's user.role, which v3 passed (§8.3, ADR-36).
 * v2's cheaper 'role' and 'cached' modes are gone: they judged sessions a hook supplied (an API key under enableSessionForAPIKeys)
 * with the owner's role, which no better-auth admin endpoint does. [R1:SEC-r1-04] [R2:BA-r2-06] [R3:BA-r3-02]
 */
...
): PrincipalSource<ApiKeyPrincipal>; // source id 'better-auth:api-key', kinds ['api-key'] (§9.0)
 ...
 * policies) therefore deny 401 under this helper; give them a real session or stub their decisions with overrideDecisions. [A][B]
 * [R2:NEST-r2-15] [R3:BA-r3-01]
 */
...
| Runtime brands and slots     | `bridge` (protocol object on the plugin), `extension` (ExtensionDefinition brand), `failure` (AuthFailure brand), `error` (brand on `BetterAuthInfrastructureError` and `BetterAuthConfigurationError`), `enhancer` (brand on `BetterAuthGuard` and `BetterAuthScopeInterceptor` instances, which B16 looks for among the app's global enhancers, §5.4) `[R3:NEST-r3-02]`, `resolution` (readings the guard records: the invocation's own result on the invocation object, and readings by lineage position on the operation carrier, §7.8) `[R3:NEST-r3-01]`, `invocation` (per-invocation values on an invocation object), `raw-body` (Express capture), `upgrade-request` (ws), `mounted` (per-server double-mount stamp), `raw-cause` (non-enumerable slot on infrastructure errors), `test-app` (slot on the plugin handle used by `initTestApp`), `test-stamp` (`stampPrincipal()` record on an args array and its elements, §7.8). Extension units own further keys under the same prefix, for example `org:active-organization-id` and `org:active-member-role` (`./organization`) |
