...
export interface HttpRequestAccessor {
  ...
  /**
   * Optional: is the response to this request still unsent (Express: !req.res.writableEnded; Fastify: the reply of request.raw is not sent)?
   * GraphQL transports refuse a context whose request already finished, which a context object shared across requests carries (§9.2).
   * Without it, that defense is skipped. [R4:SEC-r4-03]
   */
...
export interface TransportCall {
   ...
   * alias and batched operation), except for a federation reference resolver (@ResolveReference()), whose invocation is the
   * representation (gql.getRoot()), because every representation of one `_entities` call shares that field's `info` (§9.2, T7);
   * WS = the per-message args array; RPC = the message context. Scope of authorization decisions and per-invocation values.
   * Never a connection. [R1:NEST-r1-01] [R4:NEST-r4-01]
   */
...
| T6  | `lineage`, when present: `carrier` is one of `ctx.getArgs()` and belongs to one operation (never shared across concurrent requests); `position` is distinct for distinct invocations of the operation (aliases, list items, batched operations), except that invocations of one handler plan the operation reaches as parallel entry points (federation representations of one type) may share one, and equal for every guard run of one invocation; `enclosing(args)` yields exactly the positions of the enclosing invocations, nearest first, and each of them completes its guard run before the nested invocation starts. A nested reader therefore sees the reading of its own enclosing invocation, whatever its siblings select and in whatever order their guards settle (§7.8) `[R2:NEST-r2-06]` `[R3:NEST-r3-01]` `[R4:NEST-r4-01]`. | `T-carrier`, `T-stamp-per-plan`, `T-reference-resolver`                                                                                                                                                                                                                                                                                                |
...
async resolve(r) {
  ...
  try {
    const { payload } = await jwtVerify(token, createLocalJWKSet(jwks), { issuer: this.baseURL, audience: this.baseURL });
