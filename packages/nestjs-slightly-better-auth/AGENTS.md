...
- This is a published workspace. The root entry implements the Nest module, named instances, lifecycle, service readers, route planning and authentication/authorization kernel alongside the foundation contracts. The plugin and platform subpaths contain the construction bridge and Node/Web helpers; Express and Fastify have native HTTP integration and artifact tests. The `./graphql` entry provides Apollo and Mercurius transports for HTTP operations, socket operations and federation, with native driver and artifact tests. The `./microservices` entry authenticates standalone and hybrid Nest microservices through native RPC credential carriers. The admin, organization and API-key units provide authorization.
- The maintainer decided on September 23, 2026 to release the HTTP scope as `1.0.0` rather than waiting for the complete library. WebSocket transports and the conformance testing kit (issues #543 and #545) ship as later minor versions behind their own entry points, because new entry points are additive.
...
- Do not mutate a Better Auth instance's options after construction. Keep optional platform and transport integrations, including `@nestjs/microservices`, out of the root dependency graph; artifact tests load the other entry points with that peer absent.
