...
- This is a published workspace. The root entry implements the Nest module, named instances, lifecycle, service readers, route planning and authentication/authorization kernel alongside the foundation contracts. The plugin and platform subpaths contain the construction bridge and Node/Web helpers; Express and Fastify have native HTTP integration and artifact tests, and the admin, organization and API-key units provide authorization. `./testing` provides consumer overrides through the `GUARD_CORE`, `SCOPE_CORE`, `PRINCIPAL_RESOLVER` and `POLICY_INVOKER` seams, and `./testing/conformance` provides the runner-agnostic platform, transport, principal-source and policy kits that run against every built-in unit.
- The maintainer decided on September 23, 2026 to release the HTTP scope as `1.0.0` rather than waiting for the complete library. GraphQL, WebSocket and RPC transports (issues #542–#544) ship as later minor versions behind their own entry points, because new entry points are additive. Each transport entry wires its built-in transport to `transportConformance` when it lands.
