...
- The maintainer decided on September 23, 2026 to release the HTTP scope as `1.0.0` rather than waiting for the complete library. The GraphQL transport (issue #542) ships as a later minor version behind its own entry point, because new entry points are additive. The platform, principal-source and policy kits run against every built-in unit, and the transport kit against the HTTP, Socket.IO, `ws` and RPC transports (TCP, hybrid TCP, gRPC and the Compose brokers); wiring the GraphQL transport to `transportConformance` is tracked in issue #545.
