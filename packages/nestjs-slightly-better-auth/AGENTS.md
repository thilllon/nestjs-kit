...
- This is a published workspace. The root entry implements the Nest module, named instances, lifecycle, service readers, route planning and authentication/authorization kernel alongside the foundation contracts. The plugin and platform subpaths contain the construction bridge and Node/Web helpers; Express and Fastify have native HTTP integration and artifact tests, the websockets subpath authenticates Socket.IO and raw ws gateways, and the admin, organization and API-key units provide authorization.
- The maintainer decided on September 23, 2026 to release the HTTP scope as `1.0.0` rather than waiting for the complete library. Each further transport and the conformance testing kit ships as a minor version behind its own entry point, because new entry points are additive. The `./websockets` entry (issue #543) is implemented; GraphQL and RPC transports and the conformance testing kit (issues #542, #544 and #545) follow.
