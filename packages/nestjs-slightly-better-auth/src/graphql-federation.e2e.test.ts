...
async function fixture(driver: "apollo" | "mercurius", guards = true) {
  ...
  (driver) => {
    it("authorizes each representation separately even when the driver shares info", async () => {
      ...
      try {
        ...
        expect(
          f.logs.some((line) => line.includes("W_ORG_PARAM_MISSING")),
        ).toBe(true);
    ...
    it("requires field guard coverage before exposing a subgraph", async () => {
      await expect(fixture(driver, false)).rejects.toMatchObject({
